...
class Command(BaseCommand):
    help = "Import books (books.csv) and quotes (quotes.jsonl), normalize & deduplicate authors."
    ...
    def import_quotes(self):
        file_path = os.path.join(BASE_DIR, "data/quotes.jsonl")
        ...
        with transaction.atomic():
            with open(file_path, "r", encoding="utf-8") as f:
                for line in f:
                    try:
                        row = json.loads(line)
                        raw_author = (row.get("author") or "Unknown").strip()
                        display_name = normalize_author_display(raw_author)
                        if not is_reasonable_author_name(display_name):
                            continue
                        ...
                        author = authors_by_key.get(key)
                        if author is None:
                            author, created = Author.objects.get_or_create(name=display_name, defaults={"bio": ""})
                            authors_by_key[key] = author
                            if created:
                                new_authors_count += 1
