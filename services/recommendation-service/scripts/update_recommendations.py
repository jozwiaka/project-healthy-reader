...
import json
from python_common.utils.fetch_data import fetch_all
from db.db import recommendation_engine

BOOK_URL = os.getenv("BOOK_SERVICE_URL") + os.getenv("BOOK_API_PREFIX") + "/"
RATING_URL = os.getenv("RATING_SERVICE_URL") + os.getenv("RATING_API_PREFIX") + "/"

print(BOOK_URL)
print(RATING_URL)

ratings = fetch_all(RATING_URL)
books = fetch_all(BOOK_URL)

isbns = ratings["isbn"].unique()[10:12]
user_ids = ratings["user_id"].unique()[1:3]
