...
def recommend_for_user(user_id: int):
    ...
    books = pd.read_sql(books_query, engine, params=recommended_isbns)
...
def recommend_similar_books(isbn: str):
    ...
    result = engine.execute(query, {"isbn": isbn}).fetchone()
    ...
    books_query = text(f"SELECT * FROM books WHERE isbn IN ({placeholders})")
    books = pd.read_sql(books_query, engine, params=similar_isbns)
